/**
 * Nuxt server route: proxies live transcription SSE from the caller agent.
 * GET /api/transcription/:contextId → Caller Agent SSE endpoint
...
export default defineEventHandler(async (event) => {
  ...
  const targetUrl = `${callerAgentUrl.replace(/\/$/, "")}/api/transcription/${contextId}`;

  console.log(`[SSE Proxy] Fetching upstream: ${targetUrl}`);
  ...
  event.node.req.on("close", () => {
    console.log(`[SSE Proxy] Client disconnected, aborting upstream`);
  ...
  try {
    ...
    console.error(`[SSE Proxy] Upstream fetch failed:`, fetchErr.message);
    throw createError({
      statusCode: 502,
  ...
  if (!upstream.ok) {
    ...
    console.error(
      `[SSE Proxy] Upstream error: ${upstream.status} ${upstream.statusText}`,
      body,
    );
    throw createError({
  ...
  console.log(`[SSE Proxy] Upstream connected, streaming to client`);
