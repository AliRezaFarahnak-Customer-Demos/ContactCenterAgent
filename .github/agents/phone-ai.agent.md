...
- **Transcription style prompt**: The `input_audio_transcription` config includes a `prompt` field with AI-generated **keyword lists** in the target language (e.g., `'Hej, ja, nej, faktura, betaling, konto, i morgen, godt nok, tak, hej hej'` for a Danish invoice call). The `whisper-1` model uses this as a **vocabulary hint** — keyword lists work better than full sentences for whisper-1. Keywords are generated by GPT-5.2 at tool-call time, tailored to both the language and the call topic — never hardcoded.
- **Transcription model**: Uses `whisper-1` with keyword-based prompts and ISO 639-1 language hints for speech-to-text accuracy.
- **AI-classified language codes**: The language name (e.g., "Danish"), ISO 639-1 code (e.g., "da"), and transcription phrases are all classified/generated by GPT-5.2 at tool-call time via the `language`, `languageCode`, and `transcriptionHint` parameters on `MakePhoneCall`. No hardcoded mapping exists in the codebase — the AI determines the correct values for any language.
...
| ------------------- | --------------------- | -------------- | ----------------------------------------------------------------------------------- | ------------- |
| AI Foundry          | `cog-contactcenteragent`        | Sweden Central | Shared AI resource — hosts both `gpt-5.2` and `gpt-realtime`                        | §1            |
...
- **AI Foundry**: Single `cog-contactcenteragent` resource hosts both `gpt-5.2` (admin chat) and `gpt-realtime` (voice AI). The `gpt-realtime` deployment has `dependsOn: [modelDeployment]` (the gpt-5.2 deployment) to avoid parallel deployment conflicts.
...
- **User speech**: Requires `input_audio_transcription = { model: "whisper-1" }` in the `session.update` config. Without it, the Realtime API never emits transcription events. Transcription runs asynchronously — does NOT delay AI voice responses. Uses `whisper-1` with keyword-based prompts and ISO 639-1 language hints.
